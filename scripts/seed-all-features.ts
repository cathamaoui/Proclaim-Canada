...
async function main() {
  ...
  try {
    ...
    for (let i = 1; i <= 5; i++) {
      const church = await prisma.user.upsert({
        ...
        create: {
          ...
          password: await bcrypt.hash('church2026', 10),
          name: `Test Church ${i}`,
          role: 'CHURCH',
    ...
    // Create test preachers with full profiles and ratings
    const preachers = []
    ...
    for (let i = 1; i <= 3; i++) {
      const preacher = await prisma.user.upsert({
        ...
        create: {
          ...
          password: await bcrypt.hash('preacher2026', 10),
          ...
          preacherProfile: {
            create: {
              ...
              bio: `I am a dedicated preacher with ${15 + i * 5} years of experience in ministry. I specialize in inspiring sermons and community engagement. My passion is helping churches grow spiritually and numerically.`,
      ...
      // Update preacher profile with calculated average
    ...
    // Refresh and display credentials
